...
The CLI command APIs can be used directly from a Go application, see [Spo CLI Godoc](https://godoc.org/github.com/spo-next/spo/src/api/cli).
...
```bash
$ cd $GOPATH/src/github.com/spo-next/spo/cmd/cli
$ ./install.sh
```
...
```bash
$ PROG=spo-cli source $GOPATH/src/github.com/spo-next/spo/cmd/cli/autocomplete/bash_autocomplete
```
...
COMMANDS:
     ...
     addressGen            Generate spo or bitcoin addresses
     ...
     verifyAddress         Verify a spo address
     version
     walletBalance         Check the balance of a wallet
     walletDir             Displays wallet folder address
     walletHistory         Display the transaction history of specific wallet. Requires spo node rpc.
...
### Add Private Key
Add a private key to a spo wallet.

```bash
$ spo-cli addPrivateKey [command options] [private key]
```

```
OPTIONS:
    -f value [wallet file or path] private key will be added to this wallet
    if not specified then default wallet ($HOME/.spo/wallets//wallets/spo_cli.wlt)
    will be used
```

#### Example
```bash
$ spo-cli addPrivateKey -f $WALLET_PATH $PRIVATE_KEY
```
...
```bash
$ spo-cli addressBalance [addresses]
```
...
Generate new spo or bitcoin addresses.

```bash
$ spo-cli addressGen [command options] [arguments...]
```
...
OPTIONS:
        ...
        --bitcoin, -b            Output the addresses as bitcoin addresses instead of spo addresses
...
#### Examples
##### Generate `n` number of spo addresses
```bash
$ spo-cli addressGen --count 2
```
...
```bash
$ spo-cli addressGen --count 2 --bitcoin
```
...
```bash
$ spo-cli addressGen --count 2 --hide-secret
```
...
```bash
$ spo-cli addressGen --count 2 --only-addr
```
...
```bash
$ spo-cli addressGen --count 2 --seed "my super secret seed"
```
...
```bash
spo-cli addressGen --count 2 --hex
```
...
```bash
$ spo-cli addressOutputs [address list]
```

#### Example
```bash
spo-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ spo-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ spo-cli blocks 41 42
```
...
Checks if the given database file contains valid spo blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ spo-cli checkdb [db path]
```

#### Example
```bash
$ spo-cli checkdb $DB_PATH
```
...
```bash
$ spo-cli createRawTransaction [command options] [to address] [amount]
```
...
```bash
$ spo-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to a specific change address

```bash
$ spo-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ spo-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
```bash
$ spo-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ spo-cli decodeRawTransaction [raw transaction]
```

Decode a raw spo transaction.

#### Example

```bash
spo-cli decodeRawTransaction 
```
...
Broadcast a raw spo transaction.
Output is the transaction id.

```bash
$ spo-cli broadcastTransaction [raw transaction]
```

```bash
$ spo-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Generate a wallet
Generate a new spo wallet.

```bash
$ spo-cli generateWallet [command options]
```
...
OPTIONS:
        ...
        -f value  [walletName] Name of wallet. The final format will be "yourName.wlt".
                             If no wallet name is specified a generic name will be selected. (default: "spo_cli.wlt")
...
```bash
$ spo-cli generateWallet -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ spo-cli generateWallet -f "secret_wallet.wlt"
```
...
```bash
$ spo-cli generateWallet -l "cli wallet"
```
...
Generate new addresses for a spo wallet.

```bash
$ spo-cli generateAddresses [command options]
```

```
OPTIONS:
        -n value    [numberOfAddresses]  Number of addresses to generate (default: 1)
        -f value    [wallet file or path] Generate addresses in the wallet (default: $HOME/.spo/wallets//wallets/spo_cli.wlt)
        --json, -j  Returns the results in JSON format
```

##### Generate an address for the default wallet
```bash
$ spo-cli generateAddresses
```
...
```bash
$ spo-cli generateAddresses $WALLET_PATH
```
...
##### Generate a JSON output
```bash
$ spo-cli generateAddresses --json
```
...
### Last blocks
Show the last `n` spo blocks.
By default the last block is shown.

```bash
$  spo-cli lastBlocks [numberOfBlocks]
```
...
List addresses in a spo wallet.

```bash
$ spo-cli listAddresses [walletName]
```

If no `walletName` is given then default wallet ($HOME/.spo/wallets/spo_cli.wlt) is used.

> NOTE: The wallet name `spo_cli.wlt` or full path `$HOME/.spo/wallets/spo_cli.wlt` can be used.
        When only the wallet name is given then the default wallet dir, $HOME/.$COIN/wallets is used.

#### Examples
##### List addresses of default wallet
```bash
$ spo-cli listAddresses
```
...
```bash
$ spo-cli listAddresses $WALLET_NAME or $WALLET_PATH
```
...
### List wallets
List wallets in the spo wallet directory.
...
### Send
Make a spo transaction.

```bash
$ spo-cli send [command options] [to address] [amount]
```

```
OPTIONS:
        -f value    [wallet file or path] From wallet. If no path is specified your default wallet
                    (`$HOME/.spo/wallets/spo_cli.wlt`) path will be used.
        -a value    [address] From address
...
```bash
$ spo-cli send -f $WALLET_PATH -a $FROM_ADDRRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ spo-cli send -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to multiple addresses
```bash
$ spo-cli send -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
##### Generate a JSON output
```bash
$ spo-cli send -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ spo-cli transaction [transaction id]
```
...
### Verify address
Verify whether a given address is a valid spo addres or not.

```bash
$  spo-cli verifyAddress [spo address]
```
...
```bash
$ spo-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ spo-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
Check the wallet a spo wallet.

```bash
$ spo-cli walletBalance [wallet]
```

> NOTE: Both the full wallet path or only the wallet name can be used.
        If no wallet is specified then the default wallet: `$HOME/.$COIN/wallets/spo_cli.wlt` is used.
...
```bash
$ spo-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ spo-cli walletBalance ~/.spo/wallets/2018_04_01_198c.wlt
```
...
Get the current spo wallet directory.

```bash
$ spo-cli walletDir [command options]
```
...
```bash
$ spo-cli walletHistory [command options]
```
...
##### Default wallet
```bash
$ spo-cli walletHistory
```

##### Specific wallet
```bash
$ spo-cli walletHistory -f $WALLET_NAME
```
*OR*
```bash
$ spo-cli walletHistory -f $WALLET_PATH
```
...
```bash
$ spo-cli walletOutputs [wallet file]
```

#### Examples
##### Default wallet
```bash
$ spo-cli walletOutputs
```

##### Specific wallet
```bash
$ spo-cli walletHistory $WALLET_NAME
```
*OR*
```bash
$ spo-cli walletHistory $WALLET_PATH
```
...
### CLI version
Get version of current spo cli.

```bash
$ spo-cli version [command options]
```
