	...
	tt := []struct {
	...
	}{
		{
			...
			func(t *testing.T, v []byte) {
				...
				// Confirms the wallet type is spo
				require.Equal(t, "spo", w.Meta["coin"])
...
func TestLiveSend(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			errMsg:  []byte("ERROR: Transaction has zero coinhour fee. See 'spo-cli send --help'"),
			checkTx: func(t *testing.T, txid string) {},
		},
	}
	...
	errMsg := []byte("ERROR: invalid amount, too many decimal places. See 'spo-cli send --help'")
...
func TestLiveCreateAndBroadcastRawTransaction(t *testing.T) {
	...
	// Send with too small decimal value
	errMsg := []byte("ERROR: invalid amount, too many decimal places. See 'spo-cli createRawTransaction --help'")
	for i := uint64(1); i < uint64(20); i++ {
		v, err := droplet.ToString(i)
		require.NoError(t, err)
		name := fmt.Sprintf("send %v", v)
...
func TestStableGenerateWallet(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			...
			checkWallet: func(t *testing.T, w *wallet.Wallet) {
				// Confirms the default wallet name is spo_cli.wlt
				require.Equal(t, "spo_cli.wlt", w.Filename())
		...
		},
		{
			...
			errMsg: []byte("ERROR: integration-test.wlt already exist. See 'spo-cli generateWallet --help'"),
		},
	}
