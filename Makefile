...
test: ## Run tests for Spo
	go test ./cmd/... -timeout=5m
	go test ./src/... -timeout=5m

test-386: ## Run tests for Spo with GOARCH=386
	GOARCH=386 go test ./cmd/... -timeout=5m
	GOARCH=386 go test ./src/... -timeout=5m

test-amd64: ## Run tests for Spo with GOARCH=amd64
...
build-libc: configure-build ## Build libspo C client library
	rm -Rf $(BUILDLIB_DIR)/*
	go build -buildmode=c-shared  -o $(BUILDLIB_DIR)/libspo.so $(LIB_FILES)
	go build -buildmode=c-archive -o $(BUILDLIB_DIR)/libspo.a  $(LIB_FILES)
	mv $(BUILDLIB_DIR)/libspo.h $(INCLUDE_DIR)/

test-libc: build-libc ## Run tests for libspo C client library
	cp $(LIB_DIR)/cgo/tests/*.c $(BUILDLIB_DIR)/
	$(CC) -o $(BIN_DIR)/test_libspo_shared $(BUILDLIB_DIR)/*.c -lspo                    $(LDLIBS) $(LDFLAGS)
	$(CC) -o $(BIN_DIR)/test_libspo_static $(BUILDLIB_DIR)/*.c $(BUILDLIB_DIR)/libspo.a $(LDLIBS) $(LDFLAGS)
	$(LDPATHVAR)="$(LDPATH):$(BUILD_DIR)/usr/lib"                 $(BIN_DIR)/test_libspo_static
	$(LDPATHVAR)="$(LDPATH):$(BUILD_DIR)/usr/lib:$(BUILDLIB_DIR)" $(BIN_DIR)/test_libspo_shared
...
cover: ## Runs tests on ./src/ with HTML code coverage
	go test -cover -coverprofile=cover.out -coverpkg=github.com/spo-next/spo/... ./src/...
...
install-deps-libc: configure-build ## Install locally dependencies for testing libspo
...
format: ## Formats the code. Must have goimports installed (use make install-linters).
	goimports -w -local github.com/spo-next/spo ./cmd
	goimports -w -local github.com/spo-next/spo ./src
	goimports -w -local github.com/spo-next/spo ./lib
