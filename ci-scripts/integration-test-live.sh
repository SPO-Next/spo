...
# Runs "live"-mode tests against a spo node that is already running
...
# The spo node must be run with -enable-wallet-api=true
...
echo "checking if spo node is running"

http_proxy="" https_proxy="" wget -O- $HOST 2>&1 >/dev/null

if [ ! $? -eq 0 ]; then
    echo "Spo node is not running on $HOST"
    exit 1
fi
...
SPO_INTEGRATION_TESTS=1 SPO_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR SPO_NODE_HOST=$HOST \
