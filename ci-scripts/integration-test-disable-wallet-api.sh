#!/bin/bash
# Runs "disable-wallet-api"-mode tests against a spo node configured with -enable-wallet-api=false
...
DATA_DIR=$(mktemp -d -t spo-data-dir.XXXXXX)
...
# Compile the spo node
# We can't use "go run" because this creates two processes which doesn't allow us to kill it at the end
echo "compiling spo"
go build -o "$BINARY" -ldflags "${GOLDFLAGS}" cmd/spo/spo.go

# Run spo node with pinned blockchain database
echo "starting spo node in background with http listener on $HOST"

./spo-integration -disable-networking=true \
...
SPO_INTEGRATION_TESTS=1 SPO_INTEGRATION_TEST_MODE=$MODE SPO_NODE_HOST=$HOST WALLET_DIR=$WALLET_DIR \
...
SPO_INTEGRATION_TESTS=1 SPO_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
...
echo "shutting down spo node"

# Shutdown spo node
kill -s SIGINT $SPO_PID
wait $SPO_PID
